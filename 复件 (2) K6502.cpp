...
// Address
// (Indirect,X)
#define AA_IX    K6502_ReadZpW( K6502_ReadPC( PC++ ) + X )
// (Indirect),Y
#define AA_IY    K6502_ReadZpW( K6502_ReadPC( PC++ ) ) + Y
// Zero Page
#define AA_ZP    K6502_ReadPC( PC++ )
// Zero Page,X
#define AA_ZPX   (BYTE)( K6502_ReadPC( PC++ ) + X )
// Zero Page,Y
#define AA_ZPY   (BYTE)( K6502_ReadPC( PC++ ) + Y )
// Absolute
...
//byCode = *( PAGE + PC++ - 0x8000);
    ...
    switch ( byCode )
    {
      ...
      case 0x4C: // JMP Abs
		...
		{
			wA0 = ROMBANK2[ PC++ & 0x3fff ];
			PC = wA0 | (WORD)ROMBANK2[ PC & 0x3fff ] << 8;
		}
		else if( PC >= 0x8000 )
		{
			wA0 = ROMBANK0[ PC++ & 0x3fff ];
			PC = wA0 | (WORD)ROMBANK0[ PC & 0x3fff ] << 8;
		}
		else
		{
			wA0 = RAM[ PC++ ];
			PC = wA0 | (WORD)RAM[ PC ] << 8;
		}
		//wA0 = *( *ReadPC[ PC - 0x8000 ] + ( PC & 0x1fff) ); PC++;
